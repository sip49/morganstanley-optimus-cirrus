...
/**
 * An unbounded {@link TransferQueue} based on linked nodes, which allows tunable spinning. This
 * queue orders elements FIFO (first-in-first-out) with respect to any given producer. The
 * <em>head</em> of the queue is that element that has been on the queue the longest time for some
 * producer. The <em>tail</em> of the queue is that element that has been on the queue the shortest
 * time for some producer.
 *
 * <p>Beware that, unlike in most collections, the {@code size} method is <em>NOT</em> a
 ...
 * toArray} are <em>not</em> guaranteed to be performed atomically. For example, an iterator
 ...
 * placing an object into a {@code LinkedTransferQueue} <a
 * href="package-summary.html#MemoryVisibility"><i>happen-before</i></a> actions subsequent to the
 * access or removal of that element from the {@code LinkedTransferQueue} in another thread.
 *
 ...
 */
public class SpinningTransferQueue<E> extends AbstractQueue<E>
   ...
   * means that we do not use a sample header.)  If we were not
  ...
  /** */
  private static final long serialVersionUID = -5624283987264587565L;

  /** True if on multiprocessor */
  private static final boolean MP = Runtime.getRuntime().availableProcessors() > 1;

  /**
   * The number of times to spin (with randomly interspersed calls to Thread.yield) on
   * multiprocessor before blocking when a node is apparently the first waiter in the queue. See
   * above for explanation. Must be a power of two. The value is empirically derived -- it works
   * pretty well across a variety of processors, numbers of CPUs, and OSes.
   */
  // private static final int FRONT_SPINS   = 1 << 7;
  private static final long FRONT_SPINS = 1 << 40;

  /**
   * The number of times to spin before blocking when a node is preceded by another node that is
   * apparently spinning. Also serves as an increment to FRONT_SPINS on phase changes, and as base
   * average frequency for yielding during spins. Must be a power of two.
   */
  // private static final int CHAINED_SPINS = FRONT_SPINS >>> 1;
  private static final long CHAINED_SPINS = 1 << 6;

  /**
   * The maximum number of estimated removal failures (sweepVotes) to tolerate before sweeping
   * through the queue unlinking cancelled nodes that were not unlinked upon initial removal. See
   * above for explanation. The value must be at least two to avoid useless sweeps when removing
   * trailing nodes.
   */
  static final int SWEEP_THRESHOLD = 32;

  /**
   * Queue nodes. Uses Object, not E, for items to allow forgetting them after use. Relies heavily
   * on Unsafe mechanics to minimize unnecessary ordering constraints: Writes that are intrinsically
  ...
  static final class Node {
    ...
    /**
     * Links node to itself to avoid garbage retention. Called only after CASing head field, so uses
     * relaxed write.
     */
    ...
    /**
     * Sets item to self and waiter to null, to avoid garbage retention after matching or
     * cancelling. Uses relaxed writes because order is already constrained in the only calling
     * contexts: item is forgotten only after volatile/atomic mechanics that extract items.
     * Similarly, clearing waiter follows either CAS or return from park (if ever parked; else we
     * don't care).
     */
    ...
    /**
     * Returns true if this node has been matched, including the case of artificial matches due to
     * cancellation.
     */
    ...
    /**
     * Returns true if a node with the given mode cannot be appended to this node because this node
     * is unmatched and has opposite data mode.
     */
   ...
   * @return null on failure due to losing race with append in different mode, else s's predecessor,
   *     or s itself if no predecessor
   */
  ...
  private E awaitMatch(Node s, Node pred, E e, boolean timed, long nanos) {
    ...
    long spins = -1; // initialized after first item and cancel checks
    ...
    for (; ; ) {
      ...
      if (spins < 0) { // establish spins at/near front
        ...
        --spins;
        if (randomYields.nextInt((int) CHAINED_SPINS) == 0) Thread.yield(); // occasionally yield
  ...
  /**
   * Returns spin/yield value for a node with given predecessor and data mode. See above for
   * explanation.
   */
  ...
  /**
   * Returns the first unmatched node of the given mode, or null if none. Used by methods isEmpty,
   * hasWaitingConsumer.
   */
  ...
  /**
   * Traverses and counts unmatched nodes of the given mode. Used by methods size and
  ...
  final class Itr implements Iterator<E> {
    ...
    private void advance(Node prev) {
      ...
      for (Node p = prev, s, n; ; ) {
        ...
        if (s.isData) {
          if (item != null && item != s) {
            nextItem = SpinningTransferQueue.<E>cast(item);
            nextNode = s;
            return;
          }
   ...
   *
   * @param pred a node that was at one time known to be the predecessor of s, or null or s itself
   *     if s is/was at head
  ...
  public SpinningTransferQueue() {}

  /**
   * Creates a {@code LinkedTransferQueue} initially containing the elements of the given
   * collection, added in traversal order of the collection's iterator.
   *
   * @param c the collection of elements to initially contain
   * @throws NullPointerException if the specified collection or any of its elements are null
   */
  public SpinningTransferQueue(Collection<? extends E> c) {
    this();
    addAll(c);
  }
  ...
  /**
   * Inserts the specified element at the tail of this queue. As the queue is unbounded, this method
   * will never block or return {@code false}.
   *
   ...
   *
   * <p>More precisely, transfers the specified element immediately if there exists a consumer
   * already waiting to receive it (in {@link #take} or timed {@link #poll(long, TimeUnit) poll}),
   ...
   * <p>More precisely, transfers the specified element immediately if there exists a consumer
   * already waiting to receive it (in {@link #take} or timed {@link #poll(long, TimeUnit) poll}),
   * else inserts the specified element at the tail of this queue and waits until the element is
   ...
   * already waiting to receive it (in {@link #take} or timed {@link #poll(long, TimeUnit) poll}),
   * else inserts the specified element at the tail of this queue and waits until the element is
   * received by a consumer, returning {@code false} if the specified wait time elapses before the
  ...
  /**
   * Always returns {@code Integer.MAX_VALUE} because a {@code LinkedTransferQueue} is not capacity
   * constrained.
   *
  ...
  static {
    try {
      ...
      // UNSAFE = sun.misc.Unsafe.getUnsafe();
      Class k = SpinningTransferQueue.class;
